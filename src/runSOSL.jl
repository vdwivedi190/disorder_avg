...
# The PROJ_DIR must contain a job_list.txt generated by genscriptSOSL.jl


# Compute the code directory 
global codedir = length(ARGS) < 2 ? "./" : ARGS[2]
include( string(codedir,"/computeLyapunov.jl") )
include( string(codedir,"/computeSOSL.jl") ) 
...
println("Computing disorder-averaged Lyapunov exponents for the Kitaev spin liquid on the Shastry-Sutherland lattice at finite temperature...")
...
for job_index in 1:njobs
    ...
    perform_job(joblist[job_index,:], qrflag)
	flush(stdout)
end
